...
namespace aitipachong.Database.DBUtility
{
    ...
    {
        //Read the connection strings from the configuration file(web.config or app.config)
        public static readonly string ConnectionStringLocalTransaction = ConfigurationManager.AppSettings["OraConnString1"].ToString();
        public static readonly string ConnectionStringInventoryDistributedTransaction = ConfigurationManager.AppSettings["OraConnString2"].ToString();
        public static readonly string ConnectionStringOrderDistributedTrasnsaction = ConfigurationManager.AppSettings["OraConnString3"].ToString();
        public static readonly string ConnectionStringProfile = ConfigurationManager.AppSettings["OraProfileConnString"].ToString();
        public static readonly string ConnectionStringMembership = ConfigurationManager.AppSettings["OraMembershipConnString"].ToString();
        ...
        /// <returns></returns>
        public static DataSet Query(string connectionString, string commandText)
        {
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
        ...
        public static DataSet Query(string connectionString, string commandText, params OracleParameter[] cmdParms)
        {
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                OracleCommand cmd = new OracleCommand();
        ...
        private static void PrepareCommand(OracleCommand cmd, OracleConnection conn, OracleTransaction trans, CommandType cmdType, string commandText, OracleParameter[] cmdParms)
        {
            if (conn.State != ConnectionState.Open) conn.Open();
            //Set up the command
            cmd.Connection = conn;
        ...
        public static object GetSingle(string connectionString, string commandText)
        {
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
        ...
        {
            object obj = OracleHelper.GetSingle(connectionString, commandText);
            int cmdResult;
            if((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
            {
                cmdResult = 0;
