...
namespace aitipachong.XML
{
    ...
    public class XMLProcess
    {
        ...
        private XmlDocument XMLLoad()
        {
            ...
            try
            {
                if (System.IO.File.Exists(this.XMLPath)) xmlDoc.Load(this.XMLPath);
            }
            catch(Exception ex)
            {
                throw ex;
        ...
        /// <returns></returns>
        private static XmlDocument XMLLoad(string xmlFilePath)
        {
            XmlDocument xmlDoc = new XmlDocument();
            try
            {
                if (System.IO.File.Exists(xmlFilePath)) xmlDoc.Load(xmlFilePath);
            }
            catch(Exception ex)
            {
                throw ex;
        ...
        /// </summary>
        /// <param name="node">节点
        /// 示例1：XMLProcess.Read("/Node","");
        /// 示例2：XMLProcess.Read("/Node/Element[@Attribute='Name']");
        /// </param>
        /// <returns></returns>
